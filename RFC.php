 ...
 * @link       https://github.com/iamcal/rfc822
 *
 * $Id$
 */
...
class RFC
{
    ...
    private static $defaults = array(
        'allow_comments'  => true,
        'public_internet' => true   // FALSE for "strict mode"
    );
     ...
     * @return integer
     */
	public static function parseEmail($email, array $options = array())
    {
        /**
         * Any options passed into the function
         * will overide the defaults.
         */
		...
		/**
         * Changed the '*' to a '+' to require that 
         * quoted strings are not empty
         */
		$quoted_string  = "(?:$cfws?\\x22(?:$fws?$qcontent)+$fws?\\x22$cfws?)";
        ...
        /**
         * This was previously 256 based on 
         * RFC3696, but dominic's errata was accepted.
         */
		if (strlen($email) > 254) {
            return 0;
        }
		...
		if ($options['allow_comments']) {
			$email = static::removeComments($outer_comment, $email, "(x)");
		}
		 ...
		 */
		if ($options['allow_comments']) {
			$bits['local']	= static::removeComments($comment, $bits['local']);
			$bits['domain']	= static::removeComments($comment, $bits['domain']);
		}
    ...
    /**
     * Removes or replaces the comments within an email address.
     * 
     ...
     * @return string
     */
	public static function removeComments($comment, $email, $replace = '')
    {
		while (1) {
